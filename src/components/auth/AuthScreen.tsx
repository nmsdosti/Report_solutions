...
export default function AuthScreen({ onAuthenticated }: AuthScreenProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const user: User = {
      ...
      email,
      name:
        tab === "signup" ? name || email.split("@")[0] : email.split("@")[0],
    };
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-3 mb-4">
            ...
            <div className="text-left">
              <div
                className="font-black text-white text-xl tracking-wider"
                style={{ fontFamily: "system-ui" }}
              >
                TM INDUSTRIAL
              </div>
              <div className="text-[#00C9A7] text-xs tracking-[0.2em] font-medium">
                REPORT SOLUTION
              </div>
            </div>
          </div>
          <p className="text-[#4A6B8A] text-sm">
            Professional Dynamic Balancing Reports
          </p>
        </div>
        ...
        <div className="bg-[#162032] border border-[#1E3A5F] rounded-2xl p-8 shadow-2xl shadow-black/50">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  required
                  className="w-full bg-[#0D1B2A] border border-[#1E3A5F] text-white rounded-lg px-4 py-2.5 pr-10 text-sm focus:outline-none focus:border-[#00C9A7] transition-colors placeholder-[#2A4A6A]"
                />
                <button
                  type="button"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-[#00C9A7] to-[#1E90FF] text-[#0D1B2A] font-bold py-3 rounded-xl text-sm tracking-wide hover:opacity-90 active:scale-[0.98] transition-all duration-150 disabled:opacity-50 mt-2"
            >
              {loading ? (
